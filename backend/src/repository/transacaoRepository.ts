...
export class TransacaoRepository {
  ...
  async transferencia(
    ...
    if (contaOrigemId === contaDestinoId) {
      throw new AppError("Transferência para a mesma conta não permitida");
    }
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (!origem || !destino) {
        throw new AppError("Conta inválida", 404);
      }

      // 🔥 AJUSTE DE PRECISÃO: Usando métodos do Decimal para comparação
      if (origem.saldo.lessThan(valor)) {
